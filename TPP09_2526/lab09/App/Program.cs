...
public static class Program
{
    ...
    static void Consulta1()
    {
        ...
        var consulta1 = modelo.Peliculas
                            .Where(p => p.Director.StartsWith("C"))
                            .Select(p => p.Titulo);
        Show(consulta1);
    }
    ...
    static void Consulta2()
    {
        ...
        // Título y fecha de estreno de las películas disponibles en HBO Max
        var plataforma = "HBO Max";
        var consulta2 = modelo.Peliculas
                            .Where(p => p.Plataforma.Nombre == plataforma)
                            .Select(p => new { p.Titulo, p.FechaEstreno });
    ...
    static void Consulta3a()
    {
        ...
        var consulta3a = modelo.Plataformas
                            .Where(p => p.Peliculas.Count(p => p.Duracion > 120) > 1)
                            .Select(p => p.Nombre);
    ...
    static void Consulta3b()
    {
        ...
        var consulta3b = modelo.Plataformas
                            .Where(p => p.Peliculas.Any(p => p.Duracion > 120))
                            .Select(p => p.Nombre);

        Show(consulta3b);
    }

    static void Consulta3c()
    {
        // Obtener el nombre de las plataformas que tengan más de una película con una duración superior a 120 minutos.
        Console.WriteLine("Consulta 3c");

        var consulta3c = modelo.Peliculas
                            .Where(p => p.Duracion > 120)
    ...
    {
        // Obtener el nombre de las plataformas en las que ninguna película pertenezca al género Fantasy.
        Console.WriteLine("Consulta 4a");

        var consulta4a = modelo.Peliculas
                            .GroupBy(p => p.Plataforma)
                            .Where(g => g.All(p => !p.Generos.Contains("Fantasy")))
                            .Select(g => g.Key);

        Show(consulta4a);
        Console.WriteLine("Consulta 4b");

        var consulta4b = modelo.Plataformas
                            .Where(p => !p.Peliculas.Any(p => p.Generos.Contains("Fantasy")))
                            .Select(p => p.Nombre);
